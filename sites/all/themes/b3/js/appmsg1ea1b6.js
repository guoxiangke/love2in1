...
timeout: 1e3,
complete: function(n) {
has_click[i] = !1, u && (u.style.display = "none"), e == "5" ? location.href = "/mp/profile?source=from_ad&tousername=" + t + "&ticket=" + s + "&uin=" + uin + "&key=" + key + "&__biz=" + biz + "&mid=" + mid + "&idx=" + idx + "&tid=" + i : location.href = get_url(t, "source=4&tid=" + i + "&idx=" + idx + "&mid=" + mid + "&appuin=" + biz);
},
async: !0
});
}
...
var ISWP = !!navigator.userAgent.match(/Windows\sPhone/i), sw = 0, imgs_info = {}, abtest_param = "&ab_test_id=color&ab_test_value=" + (abtest == 1 ? "blue" : "gray");
...
!i.show && e < s + i.height && e + height > s && (i.el.setAttribute("src", i.src), imgs_info[t].push(i.src), i.show = !0, i.el.style.setProperty("height", "auto", "important"), i.el.style.setProperty("visibility", "visible", "important")), ISWP && i.el.width * 1 > sw && (i.el.width = sw);
}
}
function onScroll() {
...
if (e + innerHeight >= n) {
var r = performance.getEntries(), i = new Image, s = "http://mp.weixin.qq.com/mp/jsreport?type=avg", o = "24001", u = 0, a = 0, f = 0, l = 0, c = [], h = [], p = {};
for (var d = 0, v = r.length; d < v; ++d) {
var m = r[d], g = m.name;
!!g && m.initiatorType == "img" && (g.indexOf("http://mmbiz.qpic.cn") == 0 && (a++, l += m.duration), u++, f += m.duration, p[g] = {
startTime: m.startTime,
responseEnd: m.responseEnd
});
}
if (u > 0 || a > 0) {
u > 0 && (h.push(o + "_0=" + Math.round(f / u)), h.push(o + "_1=" + u), h.push(o + "_2=" + Math.round(f))), a > 0 && (h.push(o + "_3=" + Math.round(l / a)), h.push(o + "_4=" + a), h.push(o + "_5=" + Math.round(l)));
var y = 0, b = 0, w = 0;
for (var d in imgs_info) if (imgs_info.hasOwnProperty(d)) {
var E = imgs_info[d], S = 0, x = Infinity, T = 0, N = Infinity, C = !1, k = !1;
for (var L = 0, A = E.length; L < A; ++L) {
var g = E[L], O = p[g];
!O || (C = !0, S = Math.max(S, O.responseEnd), x = Math.min(x, O.startTime), g.indexOf("http://mmbiz.qpic.cn") == 0 && (T = Math.max(T, O.responseEnd), N = Math.min(N, O.startTime), k = !0));
}
C && (b += Math.round(S - x)), k && (w += Math.round(T - N));
}
h.push(o + "_6=" + b), h.push(o + "_7=" + w), i.src = s + "&r=" + Math.random() + "&" + h.join("&");
}
